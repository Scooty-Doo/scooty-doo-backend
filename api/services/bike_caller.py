...
async def start_trip(bike_id: int, user_id: int, trip_id: int) -> BikeTripStartData:
    ...
    async with httpx.AsyncClient() as client:
        try:
            ...
            return BikeTripStartData(**response_data)
        ...
        try:
            response = await client.post(
                ...
                json=BikeTripEndRequest(
                    maintenance=maintenance, ignore_zone=ignore_zone
                ).model_dump(),
                timeout=30,
            )
            response.raise_for_status()
            response_data = response.json()
            return BikeTripEndData(**response_data)
        except httpx.HTTPStatusError as exc:
            error_detail = exc.response.json().get("detail", str(exc))
            raise BikeRejectedError(error_detail)
        except httpx.RequestError as exc:
