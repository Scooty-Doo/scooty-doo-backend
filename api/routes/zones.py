...
async def get_zone_types(
    ...
    return JsonApiResponse(
        data=[ZoneTypeResource.from_db_model(zone_type, collection_url) for zone_type in zone_types],
        links=JsonApiLinks(self_link=collection_url),
    )

@router.post("/types", response_model=JsonApiResponse[ZoneTypeResource], status_code=status.HTTP_201_CREATED)
